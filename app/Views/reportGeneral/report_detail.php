                                ...
                                <div class="row">
                                    <div class="col s12 ">
                                        <ul class="collapsible collapsible-filter" style="box-shadow: none;">
                                            <li class="active">
                                                <div class="collapsible-header"><i class="material-icons">search</i>Filtrar Reporte Facturación Detallado</div>
                        ...
                        <div class="row">
                            <div class="col s12">
                                ...
                                <span class="card-title" style="margin-top: 20px;margin-bottom: 0px;">
                                        <a href="<?= base_url() . '/report_detail/excel' ?>"
                                     ...
                                     Reporte de facturaión detallado por productos<br>
                                    </span>
                                <small style="margin-bottom: 10px; display: block;">En este Reporte se detallan productos facturados. Va a nivel de producto y factura.</small>
                                <div class="divider"></div>
                                <div class="table-response" style="overflow-x:auto;">
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: center;">
                                                $ <?= $item->free_of_charge_indicator == 'false' ? number_format($item->line_extension_amount, '2', ',', '.') : '0' ?></td>
                                            <td style="text-align: center;"><?= $taxes['percentIva'] ?> %</td>
                                            <td style="text-align: center;">
                                            ...
                                            <td style="text-align: center;">
                                                $ <?= number_format($taxes['reteIVA'], '2', ',', '.') ?></td>
                                            <td style="text-align: center;">
                                                $ <?= number_format(($item->free_of_charge_indicator == 'false' ? $item->line_extension_amount : 0) + $taxes['iva'] - ($taxes['reteFuente'] + $taxes['reteICA'] + $taxes['reteIVA']), '2', ',', '.') ?></td>
                                            <?php endforeach; ?>
                                        </tbody>
                                    </table>
...
<script type="text/javascript">
    const vue = new Vue({
        ...
        mounted() {
            ...
            this.seller();
            this.user();
            if (localStorage.getItem('filters_detail')) {
                const data              = JSON.parse(localStorage.getItem('filters_detail'));
        ...
        methods: {
            send() {
                ...
                localStorage.setItem('filters_detail', JSON.stringify(data));
                axios.post(`${localStorage.getItem('url')}/report_detail`, data).then((response) => {
                    location.reload();
                });
            },
            customer() {
            ...
            reset() {
                if (localStorage.getItem('filters_detail')) {
                    localStorage.removeItem('filters_detail');
                    const url = localStorage.getItem('url');
                    location.href = url + '/report_detail/reset'
                }
