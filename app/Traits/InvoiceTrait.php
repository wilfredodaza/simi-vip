...
/**
 * Trait encargado de armar el archivo JSON para los documentos de
 * facturaci�n, nota cr�dito y nota d�bito.
...
trait InvoiceTrait
{
    ...
    /**
     * Consulta de base de datos el cual traer toda la informaci�n de la factura.
    ...
    public function request(int $id)
    {
        ...
        $this->_invoice = $model
            ->select([
                ...
                'customers.type_liability_id as customer_type_liability_id',
                'invoices.id',
                'companies.token',
                ...
                'invoices.notes',
                'invoices.created_at',
                'companies.template_pdf_id',
                'invoices.idcurrency',
                'invoices.calculationratedate',
                'invoices.calculationrate',
		'config.quantity_decimal',
		'config.economic_activity',
		'config.name_type_doc_id',
            ])
            ->join('customers', 'customers.id = invoices.customers_id')
            ->join('companies', 'companies.id = invoices.companies_id')
  	    ->join('config', 'companies.id = config.companies_id', 'left')
    ...
    /**
     * M�todo encargado de agrupar los demas m�todos para la contrucci�n de archivo json segun el tipo de documento
     * @param int $id id de la factura
     * @param string $resolution id de la resoluci�n
     * @param string $companyId i de la compa�ia
     * @param string $previsualization [ true = es previsualizado, false = factura ]
     * @return array|void
     */
    public function createDocument($id, $resolution, $companyId, $previsualization = false)
    {
        $this->request($id);
        if ($this->_invoice->type_documents_id == 1 || $this->_invoice->type_documents_id == 2) {
            $this->resolution($resolution, [1, 2], $companyId, $previsualization);
            $this->_invoice();
            $this->_customer();
            $this->_paymentForm();
            ...
            $this->taxTotal();
            return $this->_data;
        } else if ($this->_invoice->type_documents_id == 4) {
            $this->resolution($resolution, [4], $companyId,  $previsualization);
     ...
     //       $this->_paymentForm();
            ...
            $this->taxTotal();
            return $this->_data;
        } else if ($this->_invoice->type_documents_id == 5) {
            $this->resolution($resolution, [5], $companyId,  $previsualization);
            $this->billing_reference();
            $this->_invoice();
            $this->_customer();
            //       $this->_paymentForm();
            $this->_monetaryTotals('requested_monetary_totals');
            $this->_lineInvoice('debit_note_lines');
            $this->withHoldingTaxTotal();
            $this->taxTotal();
            return $this->_data;
        }
    ...
    /**
     * Este m�todo esta encargado de crear los datos principales de la factura.
     * @return void
     */
    private function _invoice()
    {
        ...
        $this->_data['template']                  = $this->_invoice->template_pdf_id;
        $this->_data['calculationrate']           = $this->_invoice->calculationrate;
        $this->_data['calculationratedate']       = $this->_invoice->calculationratedate;
        $this->_data['idcurrency']                = $this->_invoice->idcurrency;
	    if(isset($this->_invoice->quantity_decimal) && !is_null($this->_invoice->quantity_decimal)) {
  		    $this->_data['decimal']               = $this->_invoice->quantity_decimal;
	    }
	    if(isset($this->_invoice->economic_activity) && !is_null($this->_invoice->economic_activity)) {
  		    $this->_data['actividadeconomica']    = $this->_invoice->economic_activity;
	    }
	    if(isset($this->_invoice->name_type_doc_id) && !is_null($this->_invoice->name_type_doc_id)) {
  		    $this->_data['nombretipodocid']       = $this->_invoice->name_type_doc_id;
	    }
    }

    /**
     * Este m�todo esta encargado de agrupar  los datos de cliente
     * @return void
     */
    private function _customer()
    {
        ...
        $this->_data['customer']['type_organization_id']              = $this->_invoice->customer_type_organizations_id;
        ...
        $this->_data['customer']['type_liability_id']                 = $this->_invoice->customer_type_liability_id;
    }

    /**
     * M�todo encargado de la forma de pago del la factura de venta
     * @return void
     */
    ...
    /**
     * M�todo encargado de agrupar los totales de la factura.
     * @param string $type
     * @return void
     */
    public function _monetaryTotals(string $type)
    {
        $this->_data[$type]['line_extension_amount'] = (float) $this->_invoice->line_extesion_amount;
        $this->_data[$type]['tax_exclusive_amount'] = (float) $this->_invoice->tax_exclusive_amount;
        $this->_data[$type]['tax_inclusive_amount'] = (float) $this->_invoice->tax_inclusive_amount;
        $this->_data[$type]['allowance_total_amount'] = (float) $this->_invoice->allowance_total_amount;
        $this->_data[$type]['charge_total_amount'] = (float) $this->_invoice->charge_total_amount;
        $this->_data[$type]['payable_amount'] = (float) $this->_invoice->payable_amount;
    }

    /**
     * M�todo encargado de agrupar las lines de facturai�n y de producto
     * @param string $type
     * @return void
     */
    ...
    /**
     * M�todo encargado del cosecutivo de los documento electr�nicos
     * @param string $resolutions Id de la resoluci�n
     * @param string $typeDocument Id del tipo de documento a crear
     * @param string $companyId  Id de la campa�ia
     * @param string $previsualization SI es true crea un previsualizado de lo contrario crea  la factura
     * @return void
     */
    public function resolution($resolutions, $typeDocument, $companyId, $previsualization = false)
    {
        ...
        if ($previsualization) {
            ...
            if ($resolutions != 0 && $this->_invoice->resolution == null) {
                ...
                $model = $invoice->select(['resolution'])
                    ...
                    ->whereIn('companies_id', $controllerHeadquarters->idsCompaniesHeadquarters())
                    ->where([
    ...
    /**
     * Metodo encargado de agrupar los impuestos por producto
     * @param int $lineInvoiceId id de la linea de factura
     * @return array
     */
    ...
    /**
     * M�todo de agrupar impuetos y retenciones por porcentage y tipo de impuesto.
     * @param array $data array con los datos de los impuestos a grupar.
     * @param string $type ['tax_tatals' = tatal de impuestos, with_holding_tax_total= total de retenciones.]
     * @return void
     */
    ...
    /**
     * Metodo encargado de montar los impuestos totales de la factura.
     * @return void
     */
    ...
    /**
     * Metodo encargado de montar las retenciones totales de la factura.
     * @return void
     */
    ...
    /**
     * M�todo encargado de referenciar los documentos a realizar ajuste
     * Ejemplo una factura que nesecita una nota cr�dito y nota d�bito
     * @return void
     */
