...
class NoteCredit extends ServiceController
{
    ...
    public function create()
    {
        ...
        $invoiceLines = $invoice->credit_note_lines;
        $invoice  = new Invoice();
        $idInvoice = $invoice->insert($data);
        $this->_lineInvoice($invoiceLines, $idInvoice);
        if ($idInvoice) {
            return $this->respond(['status' => 'ok', 'message' => 'Guardado Correctamente.']);
        }
    }

    public function resolution()
    {
       return  $this->_resolution(4);
    }
