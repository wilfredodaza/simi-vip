...
class ProductProviderTable
{
    ...
    {
        $this->crudTable->where(['companies_id' => Auth::querys()->companies_id, 'kind_product_id' => 2]);
        if (session('user')->role_id == 2 || session('user')->role_id >= 3) {

            $this->crudTable->columns(['code', 'name', 'valor', 'description']);
