...
class DebitNoteXml extends DocumentXml
{
    ...
    public function header()
    {
        try {
          ...
          if($paymentExchangeRate = $this->xml->children($this->namespaces['cac'])->PaymentExchangeRate) {
            ...
            $resolution = '';
            $prefix = '';
            $caracter = $this->zero($this->xml->children($this->namespaces['cbc'])->ID);
            for($i=0;$i<strlen($caracter);$i++) {
               if(is_numeric($caracter[$i])) {
                   $resolution.= $caracter[$i];
               }else {
                   $prefix.= $caracter[$i];
               }
            ...
            $this->format['prefix']             = $prefix;
            $this->format['number']             = $resolution;
            ...
            $this->format['type_document_id']   = 5;
    ...
    public function company()
    {
        ...
        if ($this->zero($this->xml->children($this->namespaces['cac'])->AccountingCustomerParty->Party->PartyTaxScheme->children($this->namespaces['cbc'])->TaxLevelCode->attributes()->listName)) {
            ...
            if(count($typeRegimes) > 0) {
                $this->format['customer']['type_regime_id'] = $typeRegimes[0]->id;
            }  else {
                    $this->format['customer']['type_regime_id'] = null;
            }
        }
    ...
    public function invoiceLines()
    {
        try {
            $i = 0;
            foreach ($this->xml->children($this->namespaces['cac'])->DebitNoteLine as $item) {
                ...
                $this->format['debit_note_lines'][$i]['invoiced_quantity']         = $this->zero($item->children($this->namespaces['cbc'])->DebitedQuantity);
                $this->format['debit_note_lines'][$i]['line_extension_amount']     = $this->zero($item->children($this->namespaces['cbc'])->LineExtensionAmount);
                $this->format['debit_note_lines'][$i]['free_of_charge_indicator']  = $this->zero($item->children($this->namespaces['cbc'])->FreeOfChargeIndicator);

                $l = 0;
                foreach ($item->children($this->namespaces['cac'])->AllowanceCharge as $item2) {
                    $this->format['debit_note_lines'][$i]['allowance_charges'][$l]['charge_indicator']             = $this->zero($item2->children($this->namespaces['cbc'])->ChargeIndicator);
                    $this->format['debit_note_lines'][$i]['allowance_charges'][$l]['allowance_charge_reason']      = $this->zero($item2->children($this->namespaces['cbc'])->AllowanceChargeReason);
                    $this->format['debit_note_lines'][$i]['allowance_charges'][$l]['amount']                       = $this->zero($item2->children($this->namespaces['cbc'])->Amount);
                    $this->format['debit_note_lines'][$i]['allowance_charges'][$l]['base_amount']                  = $this->zero($item2->children($this->namespaces['cbc'])->BaseAmount);
                    $l++;
                }
                ...
                foreach ($item->TaxTotal as $item3) {
                    ...
                    $this->format['debit_note_lines'][$i]['tax_totals'][$t]['tax_id']          = (int)$this->zero($item3->TaxSubtotal->TaxCategory->TaxScheme->children($this->namespaces['cbc'])->ID);
                    $this->format['debit_note_lines'][$i]['tax_totals'][$t]['tax_amount']      = $this->zero($item3->TaxSubtotal->children($this->namespaces['cbc'])->TaxAmount);
                    $this->format['debit_note_lines'][$i]['tax_totals'][$t]['percent']         = $this->zero($item3->TaxSubtotal->TaxCategory->children($this->namespaces['cbc'])->Percent);
                    $this->format['debit_note_lines'][$i]['tax_totals'][$t]['taxable_amount']  = $this->zero($item3->TaxSubtotal->children($this->namespaces['cbc'])->TaxableAmount);
                    $t++;
                }
                ...
                $this->format['debit_note_lines'][$i]['description']   = $this->zero($item->Item->children($this->namespaces['cbc'])->Description);
                $this->format['debit_note_lines'][$i]['code']          = $item->Item->StandardItemIdentification ? $this->zero($item->Item->StandardItemIdentification->children($this->namespaces['cbc'])->ID) : NULL;
                $this->format['debit_note_lines'][$i]['price_amount']  = $this->zero($item->Price->children($this->namespaces['cbc'])->PriceAmount);
                $this->format['debit_note_lines'][$i]['base_quantity'] = $this->zero($item->Price->children($this->namespaces['cbc'])->BaseQuantity);
                $i++;
    ...
    {
        try {
            $this->format['requested_monetary_totals']['line_extension_amount']     = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->LineExtensionAmount);
            $this->format['requested_monetary_totals']['tax_exclusive_amount']      = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->TaxExclusiveAmount);
            $this->format['requested_monetary_totals']['tax_inclusive_amount']      = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->TaxInclusiveAmount);
            $this->format['requested_monetary_totals']['allowance_total_amount']    = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->AllowanceTotalAmount);
            $this->format['requested_monetary_totals']['charge_total_amount']       = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->ChargeTotalAmount);
            $this->format['requested_monetary_totals']['pre_paid_amount']           = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->PrepaidAmount);
            $this->format['requested_monetary_totals']['payable_amount']            = $this->zero($this->xml->children($this->namespaces['cac'])->RequestedMonetaryTotal->children($this->namespaces['cbc'])->PayableAmount);
        }catch (\Exception $e) {
