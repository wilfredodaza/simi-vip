...
class Payroll_suarez_leon extends BaseController
{
    protected $cargue;
    protected $workers;
    protected $payroll;
    ...
    public function workers($nit, $month_payroll)
    {
        ...
        foreach ($info as $data) {
            ...
            $type_worker = $this->type_worker->like('code', $json->TIPO_DE_EMPLEADO, 'both')->asObject()->first();
            $sub_type_worker = $this->sub_type_worker->like('code', $json->SUBTIPO_DE_TRABAJADOR, 'both')->asObject()->first();
            $type_contract = $this->type_contract->where(['id' => $json->TIPO_DE_CONTRATO])->asObject()->first();
            $type_document = $this->type_document->where(['code' => $json->TIPO_DE_DOCUMENTO])->asObject()->first();
