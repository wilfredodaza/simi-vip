...
class Payroll_tyc_contadores extends BaseController
{
    public $cargue;
    public $workers;
    public $payroll;
    ...
    public function invoice($info)
    {
        foreach ($info as $data) {
            ...
            $id_invoice = $this->invoices->insert([
                ...
                'type_documents_id' => ($data->type_document_payroll == 10)?$data->type_document_payroll:(($data->type_document_payroll == 110)?110:109),
                'companies_id' => Auth::querys()->companies_id,
        ...
        foreach ($info as $data) {
            ...
            foreach ($concepts as $concept) {
                foreach ($json->conceptosPagos as $conceptosPago) {
                    ...
                    $valorFssp = 0;
                    $descuento = 0;
                    $documentPayroll = ($data->type_document_payroll == 10)?$data->type_document_payroll:(($data->type_document_payroll == 110)?110:109);
