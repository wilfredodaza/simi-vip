...
class Payroll_sinergia extends BaseController
{
    ...
    public function workers($nit, $month_payroll)
    {
        ...
        foreach ($info as $data) {
            ...
            $type_document = $this->type_document->where(['code' => $json->TIPO_DOCUMENTO])->asObject()->first();
            ...
            if (is_null($worker)) {
                ...
                $data_customer_worker = [
                    ...
                    'sub_type_worker_id' => $this->functions_payroll->id_type_and_subtype_worker($json->subtipo_de_trabajo, true),
                    'bank_id' => $bank,
                    'bank_account_type_id' => $bank_account_type,
                    'type_contract_id' => $json->TIPO_CONTRATO,
                    ...
                    'integral_salary' => (strtolower($json->SALARIO_INTEGRAL) == 'no') ? 'false' : 'true',
                    'salary' => $json->SUELDO_BASICO,
    ...
    public function accrueds($info)
    {
        foreach ($info as $data) {
            ...
            // Salario LISTO
            $this->functions_payroll->save_data_accrueds(1, $json->SUELDO_BASICO, null, null, null, null,
                'SUELDO BASICO', $id_payroll);
            ...
            if ($json-> VALOR_HORAS_EXTRAS_NOCTURNAS_MENSUALES > 0) {
                $this->functions_payroll->save_data_accrueds(4, $json-> VALOR_HORAS_EXTRAS_NOCTURNAS_MENSUALES, null, null, $json->NUMERO_DE_HORAS_EXTRAS_NOCTURNAS,
                    1.75, ' HORAS EXTRAS NOCTURNAS MENSUALES', $id_payroll, null, 2);
            }
            // Horas Extras Diurnas
            if ($json-> VALOR_HORAS_EXTRAS_DIURNAS_MENSUALES > 0) {
                $this->functions_payroll->save_data_accrueds(3, $json-> VALOR_HORAS_EXTRAS_DIURNAS_MENSUALES, null, null, $json->NUMERO_DE_HORAS_EXTRAS_DIURNAS,
                    1.25, 'HORAS EXTRAS DIURNAS MENSUALES', $id_payroll, null, 1);
            }
            // Vacaciones Comunes listo
            if ($json->VACACIONES_DISFRUTADA_VALOR > 0) {
                $this->functions_payroll->save_data_accrueds(12, $json->VACACIONES_DISFRUTADA_VALOR,
                    (empty($json->VACACIONES_DISFRUTADA_FECHA_INICIO)?null:$json->VACACIONES_DISFRUTADA_FECHA_INICIO),
                    (empty($json->VACACIONES_DISFRUTADA_FECHA_FINAL)?null:$json->VACACIONES_DISFRUTADA_FECHA_FINAL),
                    $json->VACACIONES_DISFRUTADA_CANTIDAD, null, 'Vacaciones', $id_payroll);
            }
            // primas listo
            ...
            if ((int)$json->OTROS_CONCEPTOS_SALARIALES_IBC_SS > 0) {
                $this->functions_payroll->save_data_accrueds(26, (int)$json->OTROS_CONCEPTOS_SALARIALES_IBC_SS, null, null, null, null,
                    'OTROS CONCEPTOS SALARIALES IBC SS', $id_payroll);
            }
            // auxilios no salariales
            if ((int)$json->AUXILIO_DE_ALIMENTACION > 0) {
                $this->functions_payroll->save_data_accrueds(26, (int)$json->AUXILIO_DE_ALIMENTACION, null, null, null, null,
                    'AUXILIO DE ALIMENTACIÓN', $id_payroll);
            }
            if ((int)$json->AUXILIO_DE_RODAMIENTO > 0) {
                $this->functions_payroll->save_data_accrueds(26, (int)$json->AUXILIO_DE_RODAMIENTO, null, null, null, null,
                    'AUXILIO DE RODAMIENTO', $id_payroll);
            }
        }
    }
    ...
    public function deductions($info)
    {
        foreach ($info as $data) {
            ...
            if ((int)$json->PRESTAMO_EMPRESA > 0) {
                $this->functions_payroll->save_data_deductions(11, (int)$json->PRESTAMO_EMPRESA, 'PRESTAMO EMPRESA', $id_payroll);
            }
            if ((int)$json->COORSERPARK > 0) {
                $this->functions_payroll->save_data_deductions(11, (int)$json->COORSERPARK, 'COORSERPARK', $id_payroll);
            }
            if ((int)$json->PRESTAMO > 0) {
                $this->functions_payroll->save_data_deductions(11, (int)$json->PRESTAMO, 'PRESTAMO', $id_payroll);
            }
        }
    }
