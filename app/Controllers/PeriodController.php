...
class PeriodController extends BaseController 
{
    ...
    public function index() 
    {
        ...
        ->from('periods as per')
        ->join('payrolls', 'per.id = payrolls.period_id')
	  ...
	  'customers.companies_id' => Auth::querys()->companies_id,
        ...
        foreach($data->paginate(10) as $item) {
            ...
            $workers = $workerModel->select('count(*) as workers')
  		...
  		->join('customer_worker', 'customers.id = customer_worker.customer_id')
            ...
            $emiter = $emiterModel->select('count(*) as emiter')
            	->join('invoices', 'payrolls.invoice_id = invoices.id')
            	->join('customers', 'customers.id = invoices.customers_id')
  		->join('customer_worker', 'customers.id = customer_worker.customer_id')
            ->where([
                'payrolls.period_id'                => $item->id,
            ...
            $errors = $errorsModel->select('count(*) as errors')
            ->join('invoices', 'payrolls.invoice_id = invoices.id')
            ->join('customers', 'customers.id = invoices.customers_id')
  	    ->join('customer_worker', 'customers.id = customer_worker.customer_id')
            ->where([
            ...
            $forEmiter = $forEmiterModel->select('count(*) as for_emiter')
            ->join('invoices', 'payrolls.invoice_id = invoices.id')
            ->join('customers', 'customers.id = invoices.customers_id')
  	->join('customer_worker', 'customers.id = customer_worker.customer_id')
    ...
    {   
        ...
        $payrollDates =  explode(',',$this->request->getPost('payment_dates')[0]);


        $model = new Customer();
        $customers = $model->where([
            'type_customer_id'  => 3, 
	...
	//->where(['customers.id >' => 4366])
        ...
        foreach($customers as $item) {
            ...
            $period = $model
                ->asObject()
                ->find($this->request->getPost('period_id'));


            $model = new Payroll();
            $model->insert([
                'invoice_id'            => $invoice,
                'settlement_start_date' => $this->dataFirstMonthDay($period->month, $period->year),
                'settlement_end_date'   => $this->dataLastMonthDay($period->month, $period->year),
                'worked_time'           => '30',
        ...
        return redirect()->to(base_url('periods'))->with('success','');
    ...
    public function delete($id) 
    {
        ...
        if(isset($emiter->emiter) &&  $emiter->emiter == '0') {
            try{
                ...
                $sql = "UPDATE `invoices` INNER JOIN `payrolls` ON `payrolls`.`invoice_id` = `invoices`.`id` SET `invoices`.`companies_id` = NULL, `payrolls`.`period_id` = NULL,   `invoices`.`deleted_at` = '".date('Y-m-d H:s:i')."' WHERE `invoices`.`type_documents_id` = 9 AND `invoices`.`companies_id` = ".Auth::querys()->companies_id." AND `invoices`.`invoice_status_id` IN  (12, 13) AND `payrolls`.`period_id` = ".$id.";";
                $db = db_connect();
                $db->reconnect();
                $db->query($sql);
                $db->close();
                ...
                return redirect()->to(base_url('periods'))->with('success', 'La nomina fue eliminada correctamente.');
            }catch(\mysqli_sql_exception $e) {
               return redirect()->to(base_url('periods'))->with('errors', 'no se puede eliminar la  nomina ya se encuentra en proceso.');
            }
        } else {
            return redirect()->to(base_url('periods'))->with('errors', 'no se puede eliminar la  nomina ya se encuentra en proceso.');
        }
